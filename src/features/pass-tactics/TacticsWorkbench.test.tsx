...
it("미션 1을 관찰→예측→패스→공개→지원 순서로 완주한다", async () => {
  ...
  await user.click(screen.getByRole("radio", { name: /A1 → A2/ }));
  ...
  expect(screen.getByText(/D1이 .*로 움직였어요/)).toBeInTheDocument();
  ...
  await user.click(screen.getByRole("radio", { name: /A1 → c2r1/ }));
...
it("오답을 고르면 정답을 공개하지 않고 근거와 한 번의 수정 기회를 제공한다", async () => {
  ...
  // PREDICT에서 막힌 길을 고른다
  await user.click(screen.getByRole("radio", { name: /A1 → A3/ }));
  await user.click(screen.getByRole("button", { name: "생각 확인하기" }));
...
it("키보드만으로 선수와 패스 길을 선택한다", async () => {
  ...
  // 예측: 라디오에 스페이스로 선택
  const radio = screen.getByRole("radio", { name: /A1 → A2/ });
  radio.focus();
  await user.keyboard(" ");
